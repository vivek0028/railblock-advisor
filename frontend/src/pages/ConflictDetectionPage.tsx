...
export const ConflictDetectionPage: React.FC = () => {
  ...
  const handleQuickResolve = (conflictId: string) => {
    setResolvedIds(prev => new Set(prev).add(conflictId));
    setNotification(`Conflict ${conflictId} marked for automated solver resolution via Joint Bundling.`);
    setTimeout(() => setNotification(null), 4000);
  };
              ...
              >
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2 pt-2 border-t border-slate-100">
                    ...
                    <button
                      onClick={() => handleQuickResolve(c.conflict_id)}
                      className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold transition flex items-center space-x-1"
                    >
                    ...
                    <button
                      onClick={() => navigate("/optimizer")}
                      className="px-3.5 py-1.5 bg-railway-blue hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition flex items-center space-x-1"
                    >
