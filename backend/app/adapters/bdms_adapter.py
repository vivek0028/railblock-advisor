import json
from datetime import datetime
from pathlib import Path
from typing import Dict, Any, List

def get_data_dir() -> Path:
    # Try multiple standard relative locations
    current = Path(__file__).resolve()
    for parent in [current.parent.parent.parent.parent, current.parent.parent.parent]:
        candidate = parent / "data"
        if candidate.exists():
            return candidate
    return Path("/Users/vivekarya/Desktop/RailBlock Advisor/data")

class BDMSAdapter:
    """
    Mock adapter for Block Demanding and Monitoring System (BDMS).
    Simulates maintenance block requisitions submitted across railway divisions.
    NOTE: Uses synthetic demo data only. Does NOT connect to live Indian Railways BDMS.
    """
    SOURCE_NAME = "BDMS"
    FULL_NAME = "Block Demanding and Monitoring System"
    SYSTEM_TYPE = "Maintenance Block Requisition"

    def __init__(self):
        self.data_path = get_data_dir() / "maintenance_tasks.json"

    def load_bdms_requests(self) -> Dict[str, Any]:
        records: List[Dict[str, Any]] = []
        if self.data_path.exists():
            with open(self.data_path, "r", encoding="utf-8") as f:
                raw_data = json.load(f)
                for item in raw_data:
                    # Canonical transformation for BDMS block request
                    record = {
                        "task_id": item["task_id"],
                        "department": item["department"],
                        "asset_type": item["asset_type"],
                        "location": item["location"],
                        "description": item["description"],
                        "duration_hours": float(item["duration_hours"]),
                        "preferred_date": item["preferred_date"],
                        "deadline": item["deadline"],
                        "criticality": item["criticality"],
                        "overdue": bool(item.get("overdue", False)),
                        "required_resources": item.get("required_resources", []),
                        "dependencies": item.get("dependencies", []),
                        "compatible_departments": item.get("compatible_departments", []),
                        "status": item.get("status", "Pending"),
                        "data_source": self.SOURCE_NAME,
                        "data_label": "DEMO DATA"
                    }
                    records.append(record)

        metadata = {
            "source": self.SOURCE_NAME,
            "full_name": self.FULL_NAME,
            "status": "Connected (Demo Mode)",
            "record_count": len(records),
            "last_sync": "2026-09-17T10:00:00",
            "is_live_connection": False,
            "disclaimer": "Synthetic railway maintenance block requisitions for demo purposes only."
        }
        return {"metadata": metadata, "records": records}
